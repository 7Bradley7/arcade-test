...
bool arcade::LibPacman::isAvailableDirect(int x, int y)
{
    if (isAvailableGhost(map.at(ghostY + y).at(ghostX + x)) == true ||
    isAvailableGhost(map.at(ghostY - y).at(ghostX - x)) == true) {
        return true;
    }
    return false;
}
